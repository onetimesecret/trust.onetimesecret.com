...
description: The GDPR Article 32 measures behind the Onetime Secret DPA, with a shared-responsibility model.
---
...
This document describes the technical and organizational measures ("TOMs") implemented by Onetime Secret (the "Processor") pursuant to Article 32 of the GDPR to ensure a level of security appropriate to the risk. It supplements Section 4 (Security) and Schedule C of the Onetime Secret <a href={constants.urls.dpa}>Data Processing Agreement</a> ("DPA") and is the canonical description of those measures; capitalized terms used but not defined here have the meaning given in the DPA.
...
*Technical:*
- Administrative access to production systems requires multi-factor authentication and encrypted connections. Support and operational tooling is deployed per regional environment; administrative access from outside a region occurs only from Processor staff terminals connecting over an authenticated, encrypted VPN into that region. Production data is not replicated to, or stored on, infrastructure outside its region for administrative purposes.
...
- Multi-tenant infrastructure ({multiTenantTiers}) maintains secure compartmentalization between customers; Global Elite customers receive logically dedicated environments with isolated application stacks and database instances.
...
*Technical:*
- Local encrypted backups retained for {spelledDays(constants.retention.local_backup_days)}; geo-located encrypted backups (Global Elite) stored in {constants.infrastructure.geo_backup_location} with automatic expiration after {spelledDays(constants.retention.geo_backup_expiry_days)}. Both tiers fall within the Backup Retention Period defined in DPA §1.17.
...
- On account deletion, Account Data is deleted following the Operational Retention Period ({constants.retention.operational_days} days, retained to facilitate export requests), and encrypted backup copies are removed within the Backup Retention Period ({constants.retention.backup_days} days) (DPA §10).
...
- The Company is notified by email of any intended new or replacement Subprocessor and has a thirty ({constants.retention.subprocessor_notice_days})-day objection window, during which it may terminate the affected services without penalty; in an emergency, a replacement may be engaged first with prompt notice afterward, the objection window running from that notice (DPA §6).
